...
DEFAULT_OPENCODE_FREE = ["big-pickle", "nemotron-3-ultra-free", "mimo-v2.5-free", "deepseek-v4-flash-free", "nemotron-3.5-lightning-free"]
DEFAULT_GROQ_MODELS = ["llama-3.3-70b-versatile", "llama-3.1-8b-instant", "mixtral-8x7b-32768", "gemma2-9b-it", "qwen/qwen3-32b"]

def init_session():
    defaults = {"messages": [], "github_token": "", "provider": "OpenCode Zen", "api_key": "", "selected_models": [], "fetched_models": [], "last_file_content": None, "last_file_name": None, "last_files_for_zip": {}, "github_user": None, "file_expert": False, "sandbox_code": ""}
...
with st.sidebar:
    ...
    gh_token = st.text_input("GitHub Personal Access Token", type="password", value=st.session_state.github_token, help="Needs repo scope. Fine-grained tokens preferred.", key="gh_token_input")
    ...
    provider = st.radio("Provider", ["OpenCode Zen", "Groq"], index=0 if st.session_state.provider == "OpenCode Zen" else 1, horizontal=True)
    ...
    with col1:
        if st.button("Fetch Models", use_container_width=True):
            with st.spinner("Fetching..."):
                ...
                st.session_state.fetched_models = models or (DEFAULT_OPENCODE_FREE if provider == "OpenCode Zen" else DEFAULT_GROQ_MODELS)
    with col2:
        if st.button("Use Defaults", use_container_width=True):
            st.session_state.fetched_models = DEFAULT_OPENCODE_FREE if provider == "OpenCode Zen" else DEFAULT_GROQ_MODELS
    models = st.session_state.fetched_models or (DEFAULT_OPENCODE_FREE if provider == "OpenCode Zen" else DEFAULT_GROQ_MODELS)
    st.markdown("**Select models**")
    selected = []
    for m in models:
        checked = m in st.session_state.selected_models or (not st.session_state.selected_models and m == models[0])
        if st.checkbox(m, value=checked, key=f"model_{m}"): selected.append(m)
    st.session_state.selected_models = selected
    if selected: st.caption(f"Active model: {selected[0]}")
    else: st.warning("Select at least one model")
    st.markdown("---")
    st.markdown("#### Modes")
    st.session_state.file_expert = st.checkbox("File Expert mode", value=st.session_state.file_expert, help="Deeper multi-file analysis and refactor suggestions")
    st.markdown("---")
...
st.markdown("# GitHub Agent")
st.markdown('<p style="color:#555555;margin-top:-0.7rem;">Create · Read · Update · Delete repositories and files with natural language</p>', unsafe_allow_html=True)
st.caption("Use the Streamlit menu (top-right) to switch System / Light / Dark theme if needed. This app is designed for light black-and-white appearance.")
...
with st.expander("Sandbox / Notes"):
    st.markdown("This agent prefers the connected GitHub MCP tools when available. For complex multi-step workflows you can extend it with LangGraph. Code execution sandbox is not enabled in this version for security. Use the File Expert toggle for deeper analysis.")

st.markdown('<div style="margin-top:2.5rem;text-align:center;color:#888888;font-size:0.8rem;">GitHub Agent · Black & White · Groq + OpenCode Zen · MCP preferred</div>', unsafe_allow_html=True)
